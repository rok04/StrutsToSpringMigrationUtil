...
public class HtmlLinkStartParser extends AbstractTagParser implements TagParser {
	...
	public String doParse(String line, ParserContext parserContext) {
		...
		if(hasParameters(name, property, paramId, paramProperty)){
			...
			replacedString.append("\n");
			replacedString.append("\t");
			replacedString.append(addParameters(name, property, paramId,paramName,paramProperty));
			replacedString.append("\n");
			replacedString.append("</c:url>");
		}else{
			replacedString.append(" />");
	...
	private String addParameters(String name, String property, String paramId,
		...
		if(!isEmpty(paramId) && (!isEmpty(paramName) || !isEmpty(paramProperty))){
			...
			if(!isEmpty(name))
			{
				stringBuffer.append(removeLeadingAndTrailingCommas(name));
				isDotRequired = true;
			}
			...
			if(!isEmpty(property)){
				...
				{
					stringBuffer.append(".");
					stringBuffer.append(removeLeadingAndTrailingCommas(property));
				}
			}
			stringBuffer.append("}");
			stringBuffer.append(DOUBLE_QUOTE);
